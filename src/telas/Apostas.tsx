...
export function Apostas({
  registros, sugIndex, banca, onAlterar,
}: {
  registros: Registro[];
  ...
  return (
    <div className="space-y-4 overflow-x-hidden">
      ...
      {ordenadas.map((v) => (
        <CardAposta key={v.r.id} registro={v.r} estado={v.estado} pre={v.pre} banca={banca} onAlterar={onAlterar} />
      ))}
    </div>
  );
}
...
function CardAposta({
  registro, estado, pre, banca, onAlterar,
}: {
